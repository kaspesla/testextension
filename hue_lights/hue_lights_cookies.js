new(function(ext) {
    ...
    ext.lightColorHSV = function(light, h, s, v, callback) {
        sendLightColorCommand(light, {
            h: h,
            s: s,
            v: v
        }, 0);
    ...
    ext.lightColorHex = function(light, hex, callback) {
        sendLightColorCommand(light, rgb2hsv(hexToRgb(hex)), 0);
        waitAndCall(callback, noWait);
    }
    ...
    ext.lightColorHSVFade = function(light, h, s, v, fade, callback) {
        fad = fadeMap(fade)
        sendLightColorCommand(light, {
            h: h,
            s: s,
    ...
    function registerExtension() {
        ...
        var descriptor_unconfiged = {
            blocks: [
                ['w', 'Setup Philips Hue Server %s port %n', 'setServer', "10.0.1.123", "80"]
            ],
            menus: {},
        };
